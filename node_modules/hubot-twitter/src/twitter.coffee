...
class Twitter extends Adapter
 ...
 send: (user, strings...) ->
   ...
   strings.forEach (str) =>
     ...
     tweetsText.forEach (tweetText) =>
       user.user.cc && (tweetText = tweetText + " " + user.user.cc)
 ...
 run: ->
   ...
   bot.tweet self.robot.name, (data, err) ->
     ...
     console.log "received #{msg} from #{data.user.screen_name} cc #{ccUsers}"
     tmsg = new TextMessage({ user: data.user.screen_name, status_id: data.id_str, cc: ccUsers }, msg)
     self.receive tmsg
     if err
